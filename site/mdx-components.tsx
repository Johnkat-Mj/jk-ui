...
export const mdxComponents = {
    ...
    return (
      <h2
        id={props.children
          ...
          .toLowerCase()}
        className={`${cn(
          "text-fg-title scroll-mt-26 first:mt-0 mt-12 text-lg font-medium",
          className
        )}`}
        {...props}
      />
  ...
  h3: ({ className, ...props }: React.ComponentProps<"h3">) => (
    <h3
      className={`${cn(
        "text-fg-title scroll-mt-26 mt-8 first:mt-0 font-medium",
        className
      )}`}
      {...props}
    />
  ...
  p: ({ className, ...props }: React.ComponentProps<"p">) => (
    <div
      className={`${cn("mt-6 first:mt-0 text-fg leading-normal", className)}`}
      {...props}
    />
  ),
  ...
  table: ({ className, ...props }: React.ComponentProps<"table">) => (
    <div className="relative overflow-x-auto w-full rounded-lg border border-border-strong/70 p-1 mt-6">
      <table
        className={cn(
  ...
  th: ({ className, ...props }: React.ComponentProps<"th">) => (
    <th
      className={cn(
        "px-4 py-2.5 text-left font-medium text-fg-title whitespace-nowrap [[align=center]]:text-center [[align=right]]:text-right",
        className
      )}
      {...props}
    />
  ...
  tbody: ({ className, ...props }: React.ComponentProps<'tbody'>) => (
    <tbody className={cn('text-sm divide-y divide-border-strong/60',className)} {...props} />
  ),
