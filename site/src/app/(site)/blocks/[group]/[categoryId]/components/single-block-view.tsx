...
const LoadRegistrySource = dynamic(
  () => import("./load-registry-source").then((module) => module.LoadRegistrySource),
  {
    loading: () => <div className="bg-[#07090F] inner-radius p-4 min-h-40 animate-pulse" />,
  },
)
...
const SingleBlockViewComponent = ({ name, preview }: SingleBlockViewProps) => {
  ...
  const PreviewBox = () => <PreviewBlockFrame preview={preview} reloadKey={reloadKey} />
  const SourceCode = () => <LoadRegistrySource name={name} /> 
    ...
    <article className="relative w-full px-1 sm:px-4 xl:px-8">
      ...
      >
        <div className="w-full flex justify-between items-center h-12 px-3.5 relative">
          <TabList className="bg-bg shadow border border-border/50 p-0.5 rounded-md flex items-center">
        ...
        <TabPanel id={"preview"} className="flex-1">
          <div className="bg-bg inner-radius ui-preview-height ui-preview-frame-box overflow-hidden border border-gray-50 dark:border-gray-900 w-full relative group flex-1">
            <PreviewBox />
          </div>
        </TabPanel>
