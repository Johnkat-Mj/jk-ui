...
export const SiteBlockCard = ({ title, group, illustrations, categoryId }: { title: string, group: string, illustrations: { light: string, dark: string }, categoryId: string }) => {
    return (
        <Link href={`/blocks/${group}/${categoryId}`} aria-label={`Link to blocks : ${title}`}
            ...
            <div className="aspect-standard-tv bg-bg-muted/30 rounded-ui pointer-events-none">
                <Image src={illustrations.light} alt={`Illustration light ${title}`} width={1455}
                    height={1091} className="size-full object-cover dark:hidden" />
                <Image src={illustrations.dark} alt={`Illustration dark ${title}`} width={1455}
