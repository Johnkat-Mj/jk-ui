    ...
    >
      {isPending ? (
        ...
        <span
          ...
          className="iconify ph--magnifying-glass size-5 shrink-0 text-fg-muted"
        />
      )}
      <Input
...
const CommandMenuSection = <T extends object>({
  ...
  return (
    ...
    >
      {"label" in props && (
        <Header className="col-span-full mb-1 block min-w-(--trigger-width) truncate px-2.5 text-fg-muted text-xs">
          {props.label}
        </Header>
      )}
...
const renderer: CollectionRenderer = {
  CollectionRoot(props) {
    if (props.collection.size === 0) {
      return (
        <div className="col-span-full p-4 text-center text-fg-muted text-sm">No results found.</div>
      )
    }
...
const CommandMenuFooter = ({ className, ...props }: React.ComponentProps<"div">) => {
  return (
    <div
      className={twMerge(
        "col-span-full flex-none border-t px-2 py-1.5 text-fg-muted text-sm",
