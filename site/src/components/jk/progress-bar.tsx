...
export const ProgressBar = ({ className, children, ...props }: ProgressBarProps) => {
  return (
...
export const ProgressBarHeader = ({ className, ...props }: React.ComponentProps<"div">) => {
  return (
    <div
...
export const ProgressBarTrack = ({ className, ref, ...props }: React.ComponentProps<"div">) => {
    ...
    <span data-slot="progress-bar-track" className="relative block w-full">
      ...
      <div ref={ref} className="flex w-full items-center gap-x-2" {...props}>
        <div
          ...
          className={twMerge(
            "relative h-1.5 w-full min-w-52 overflow-hidden rounded-full bg-(--progress-container-bg,var(--color-bg-muted)) outline-1 outline-transparent -outline-offset-1 will-change-transform",
            className,
          )}
        >
          {!isIndeterminate ? (
            <div
              ...
              className="absolute inset-s-0 top-0 h-full rounded-full bg-current transition-[width] duration-200 ease-linear will-change-[width] motion-reduce:transition-none forced-colors:bg-[Highlight]"
              style={{ width: `${percentage}%` }}
            />
          ) : (
            <div
              data-slot="progress-content"
              className="absolute top-0 h-full animate-[progress-slide_2000ms_ease-in-out_infinite] rounded-full bg-current forced-colors:bg-[Highlight]"
              style={{ width: "40%" }}
            />
          )}
        </div>
