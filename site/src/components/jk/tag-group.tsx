...
export function Tag({ children, className, ...props }: TagProps) {
  ...
  return (
    <AriaTag
      textValue={textValue}
      className={cx(
        "[--tag-selected-bg:--alpha(var(--color-primary-100)/40%)] dark:[--tag-selected-bg:--alpha(var(--color-primary-900)/40%)] [--tag-selected-fg:var(--color-primary)]",
        "[--tag-selected-ring:var(--color-primary-300)] dark:[--tag-selected-ring:--alpha(var(--color-primary-900)/60%)]",
        "inset-ring inset-ring-border-input outline-hidden bg-bg",
    ...
    >
      {({ allowsRemoving }) => (
        <>
          {children}
          {allowsRemoving ? (
            <Button slot="remove" className="bg-fg-title text-bg -me-1 flex size-3.5 items-center-safe justify-center rounded-full in-selected:bg-primary in-selected:text-white">
