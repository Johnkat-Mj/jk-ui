...
export const Table01 = () => {
    return (
        <div className="w-full max-w-7xl px-1 sm:px-4 mx-auto py-4">
            <div className="rounded-ui border border-border-input/60">
                ...
                <Table selectionMode="multiple" className="text-left [--gutter-x:--spacing(4)] [--gutter-y:--spacing(3)]">
                    <TableColumns fillGray className="border-t border-border-input/60">
                        <TableColumn isRowHeader className="text-xs font-medium text-fg-muted uppercase tracking-wider">
                            Name
                        </TableColumn>
                            ...
                            users.map(user => <TableRow key={user.id} className="group">
                                <TableCell>
                                    <div className="flex items-center gap-3">
                                        <Avatar size="sm" text={user.initials} variant="solid" intent="solid-neutral" />
                                ...
                                <TableCell>
                                    <span className="text-fg-muted">{user.department}</span>
                                </TableCell>
                                <TableCell>
                                   
                                    <Badge variant="soft" intent={
                                        user.status==="active" ? "soft-success" : user.status==="pending" ? "soft-warning" : "soft-danger"
                                ...
                                <TableCell>
                                    <span className="text-fg-muted text-sm">{user.joined}</span>
                                </TableCell>
                    ...
                    <div className="flex-1 md:flex md:items-center md:justify-center">
                        <div className="sm:max-w-max w-full md:w-auto">
                            <InputGroup className="w-full relative">
                                <InputGroup.Leading className="text-xs text-fg-muted px-3 border-r border-border-input">
                                    Per Page
                                </InputGroup.Leading>
