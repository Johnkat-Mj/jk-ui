            ...
            <SidebarBase className="fixed h-dvh py-3 overflow-hidden w-11/12 max-w-64 md:w-64 transition-all -translate-x-full fx-open:translate-x-0 md:translate-x-0 md:transition-none  bg-bg-surface border-r border-bg-muted/70 flex flex-col justify-between px-4 ease-linear z-80">
                <div className="min-h-max py-2 border-b border-border">
                    <a href="#" className="flex items-center gap-x-3 font-semibold text-fg-subtitle">
                        <span className="size-8 bg-fg-title text-bg rounded-ui d-flex-place-center shadow-sm transition-colors duration-300">
                ...
                <nav className="flex-1 pt-6 flex flex-col">
                    <span className="mb-2 text-sm text-fg-muted uppercase">Navigation</span>
                    <ul className="text-fg-muted space-y-2 w-full">
                        {
                            items.map(item => <SidebarItem key={`sidebar-item-${item.id}`} {...item} />)
                        }
                    </ul>
                </nav>
                <div>
                    <ul className="flex flex-col gap-y-2 text-fg-muted">
                                ...
                                <Button aria-label="Open user menu" size='none' variant='ghost' className="gap-3 w-full p-1.5 border border-border-strong/40">
                                    ...
                                    <div className="flex flex-1 justify-between items-center overflow-hidden">
                                        <div className="flex flex-col -space-y-0.5 text-left flex-1 overflow-hidden">
                                            <span className="font-semibold text-sm text-fg-title truncate">
                                                Johnkat MJ
                                            </span>
                                        ...
                                        <span aria-hidden="true" className="flex min-w-max text-fg-muted opacity-60">
                                            <span className="iconify ph--caret-up-down"></span>
                                        </span>
                                    </div>
                                </Button>
