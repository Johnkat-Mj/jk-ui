    ...
    <CommandMenuContext value={{ isPending: isPending, escapeButton: escapeButton }}>
      <ModalContext value={{ isOpen: props.isOpen, onOpenChange: onOpenChange }}>
        ...
        >
          <Modal
            className={cx(
              "row-start-2 bg-popover text-start text-popover-foreground shadow-lg outline-none ring ring-muted-fg/15 md:row-start-1 dark:ring-border",
    ...
    >
      {isPending ? (
        ...
        <span
          ...
          className="iconify ph--magnifying-glass size-5 shrink-0 text-muted-foreground"
        />
      )}
      <Input
        placeholder={placeholder ?? "Search..."}
        className="w-full min-w-0 bg-transparent px-2.5 py-2 text-base text-foreground placeholder-muted-fg outline-hidden focus:outline-hidden sm:px-2 sm:py-1.5 sm:text-sm [&::-ms-reveal]:hidden [&::-webkit-search-cancel-button]:hidden"
      />
      {escapeButton && (
        <Button
          onPress={() => state?.close()}
...
const CommandMenuShortcut = ({
  ...
  <DropdownKeyboard
    className={twMerge(
      "gap-0.5 font-sans text-[10.5px] uppercase *:inset-ring *:inset-ring-muted-fg/20 *:grid *:size-5.5 *:place-content-center *:rounded-xs *:bg-background",
      className,
    )}
    {...props}
  />
