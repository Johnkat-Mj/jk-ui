...
const DisclosureItemTriggerIndicator = (props: (IndicatorIcon | IndicatorSign) & { className?: string }) => {
  if (props.type === "icon") {
    ...
    return <span aria-hidden className={cx(
      "flex text-muted-foreground iconify group-aria-expanded:rotate-180",
      "ease-linear duration-200",
      icon, props.className
    )}></span>
  }
  return <span aria-hidden className={cx(
    "flex text-muted-foreground relative size-2.5 disclosure-indicator-plus disclosure-indicator-to-minus",
    props.className
  )}></span>
}
