...
const cellStyles = tv({
  ...
  variants: {
    isSelected: {
      false: 'text-foreground hover:bg-muted pressed:bg-muted/60',
      true: 'bg-primary invalid:bg-destructive text-white forced-colors:bg-[Highlight] forced-colors:invalid:bg-[Mark] forced-colors:text-[HighlightText]'
    },
    isDisabled: {
...
export function Calendar<T extends DateValue>(
  ...
  return (
    <AriaCalendar {...props} className={cx(props.className, 'flex flex-col font-sans w-[calc(9*var(--spacing)*7)] max-w-full @container')}>
      ...
      {errorMessage && <Text slot="errorMessage" className="text-sm text-destructive">{errorMessage}</Text>}
    </AriaCalendar>
  );
}
  ...
  return (
    <header className="flex items-center gap-1 pb-4 px-1 border-box flex-1">
      ...
      <div className="flex-1">
        <Heading className="font-sans font-semibold [font-variation-settings:normal] text-base text-center mx-2 my-0 text-foreground truncate" />
      </div>
