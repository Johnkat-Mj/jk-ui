...
const generateCompoundVariants = (sides: Array<Sides>) => {
  return sides.map((side) => ({
    ...
    className:
      side === "top"
        ? "top-2 inset-x-2 rounded-ui ring-1 border-b-0"
        : side === "bottom"
          ? "bottom-2 inset-x-2 rounded-ui ring-1 border-t-0"
          : side === "left"
            ? "left-2 inset-y-2 rounded-ui ring-1 border-r-0"
            : "right-2 inset-y-2 rounded-ui ring-1 border-l-0",
  }))
}

const sheetContentStyles = tv({
  base: [
    "fixed z-50 grid border-muted-foreground/20 bg-popover text-popover-foreground shadow-lg dark:border-border",
