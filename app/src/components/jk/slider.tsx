...
export function Slider({ className, ...props }: React.ComponentProps<typeof PrimitiveSlider>) {
  return (
    <PrimitiveSlider
      data-slot="control"
      className={cx(
        "[--slider-track-bg:var(--color-bg-muted)] text-primary",
...
export function SliderOutput({
  ...
  return (
    <PrimitiveSliderOutput
      className={cx("font-medium text-muted-foreground text-base/6 sm:text-sm/6", className)}
      {...props}
    />
  )
}
...
export function SliderThumb({
  ...
  return (
    <PrimitiveSliderThumb
      className={cx(
        "top-[50%] left-[50%] size-5 rounded-full border-2 border-current bg-background outline-hidden ring-black transition-[width,height]",
        className,
      )}
      {...props}
    />
...
export function SliderFill({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <div
      ...
      style={getStyle()}
      className={twMerge(
        "group-orientation-horizontal/top-0 pointer-events-none absolute rounded-full bg-current group-disabled/track:opacity-60 group-orientation-vertical/track:bottom-0 group-orientation-horizontal/track:h-full group-orientation-vertical/track:w-full",
        className,
      )}
    />
  )
