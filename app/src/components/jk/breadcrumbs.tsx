    ...
    <BreadcrumbsContext.Provider value={contextValue}>
      <nav aria-label="Breadcrumbs" className={wrapper}>
        <BreadcrumbsPrimitive
          {...props}
          className={cx("flex items-center text-muted-foreground", SPACING_MAP[spacing], className)}
        />
      </nav>
        ...
        return (
          <>
            {href && !isCurrent ? (
              <Link
                href={href}
                className={cx("flex items-center ease-linear hover:text-muted-foreground", className)}
              >
                {content}
              </Link>
            ) : (
