        ...
        >
            ...
            </span>
            {
                href.startsWith("http") ? (
                    <span
                        aria-hidden="true"
                        className="absolute in-hover:text-primary top-(--card-padding) right-(--card-padding) text-fg-muted text-xs iconify ph--arrow-square-out"
                    />
                ) : (
                    <span
                        aria-hidden="true"
                        className="absolute in-hover:text-primary top-(--card-padding) right-(--card-padding) text-fg-muted text-xs iconify ph--arrow-up-right"
                    />
                )
            }

            <h2 className="font-medium text-lg text-fg-title mt-4">{title}</h2>
            <p className="text-sm text-fg-muted mt-1 leading-6">
                {description}
            </p>
        </UiLink>
    )
